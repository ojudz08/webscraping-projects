...
class jobScrape():
    ...
    def first_page(self):
        ...
        for item in jobcard:
            ...
            if job_tmp in job_title: 
                pass  # skip if there's duplicate job title
            else:
                job_title.append(job_tmp)
    ...
    def next_page(self, job1_list, diff_mins):
        job_title, company, job_link, location, position_type, work_arrangement, salary, posted_on = [], [], [], [], [], [], [], []
        job_dct = {}
        ...
        for pg in range(2, 100):
            ...
            for i in range(0, len(data)):
                job_tmp = data[i]['title']
                
                if job_tmp in job1_list:
                    pass  # skip if there's duplicate job title
                else:
    ...
    def get_data(self):
        ...
        if diff_mins > self.min_posted: 
            ...
            job1_list = data_1st['Position'].to_list()
            data_next = self.next_page(job1_list, diff_mins)
